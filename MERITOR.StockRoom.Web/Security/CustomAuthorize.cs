    ...
    public class CustomAuthorize : AuthorizeAttribute
    {
        ...
        public override void OnAuthorization(AuthorizationContext filterContext)
        {
            ...
            else
            {
                ...
                //CustomPrincipal mp = new CustomPrincipal("KI");

                //filterContext.Result = new RedirectToRouteResult(new RouteValueDictionary(new { Controller = "Home", Action = "Logout" }));
                if (filterContext.HttpContext.Request.IsAjaxRequest() || filterContext.HttpContext.Request.HttpMethod.ToString() == "POST")
                {
                    try
                    {
